...
- Status changes go **only** through the dedicated `PATCH /repair-order/{id}/status` (`RepairOrderStatusUpdateRequest`), restricted to `TECNICO`/`GERENTE`/`ADMIN`. The general `PUT /repair-order/{id}` ignores the body's `status` entirely.
