...
│
├── config/
│   ├── SecurityConfig.java              # Stateless JWT policy; public /auth/*; ADMIN/GERENTE create/delete on Brand/Accessory + delete on Customer/Device/RepairOrder/Payment (soft); TECNICO+ for status; ADMIN-only /user; denyAll catch-all
...
└── db/migration/V1..V19__*.sql   # confirm actual max with `ls` — append-only
```
...
- **I1.1 — Every API deletion is a soft delete.** The 6 deletable entities (`Brand`, `Accessory`, `Customer`, `Device`, `RepairOrder`, `Payment`) carry `@SoftDelete(strategy = TIMESTAMP, columnName = "deleted_at")`; `deleteById` stamps `deleted_at` and all reads (derived queries, `Specification`, `@EntityGraph`, the `devices_accessories` join table) get `deleted_at IS NULL` appended. There is **no API endpoint for permanent removal** — that is a manual DB operation. Services reject soft-deleting a record with active children (`EntityInUseException` → 409): `Brand`←`Device`, `Accessory`←`Device`, `Customer`/`Device`←`RepairOrder`, `RepairOrder`←`Payment`. A new deletable entity must add `@SoftDelete`, a `V{n}` migration adding its `deleted_at` column (plus any owned collection table), and — if it can be referenced — an in-use guard.
