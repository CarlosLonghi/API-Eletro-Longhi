...
- Mapping is manual via `mapper/*` utility classes using Lombok builders (e.g. `DeviceMapper.toDeviceEntity` builds Brand and Accessory placeholders by id). Always consult mappers when changing shape of requests/responses.
...
- To add a new request field: update the request record in `controller/request`, update mapper (e.g. `DeviceMapper.toDeviceEntity`) and service logic; add `@NotNull/@NotBlank` annotations as needed to reuse `ApplicationControllerAdvice` validation mapping.
...
- DTOs & mappers: `controller/request/*.java`, `controller/response/*.java`, `mapper/*.java`
