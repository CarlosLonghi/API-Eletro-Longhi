...
- **RepairOrder** — `@ManyToOne` to both `Customer` and `Device` (as of `V13`, a device can have more than one repair order over its lifetime; a new order for the same device is only allowed once the previous one reached `DEVICE_COLLECTED` — enforced in `RepairOrderService`, not the DB). Status is `entity/enums/RepairOrderStatus`, a workflow, not a free enum: `AWAITING_EVALUATION → IN_EVALUATION → AWAITING_APPROVAL → APPROVED → AWAITING_PARTS → IN_REPAIR → REPAIR_COMPLETED → DEVICE_COLLECTED`. There's a dedicated `PATCH`-style status endpoint using `RepairOrderStatusUpdateRequest`. Moving an order to `DEVICE_COLLECTED` requires its linked `Payment` to be `APPROVED` (`RepairOrderNotPaidException` → 422). 1:1 with `Payment` via `@OneToOne(mappedBy = "repairOrder")`; the paginated listing eager-loads it (`@EntityGraph`), filters by `paymentStatus`, and returns `paymentStatus` + `paymentId` on each row (null when the order has no payment yet).
- **Payment** — `@OneToOne` to `RepairOrder` (owns `repair_order_id`, `NOT NULL UNIQUE`): `PaymentService.save` rejects a second payment for the same order (`PaymentAlreadyExistsForRepairOrderException` → 422) and a payment for an order whose status is still before `APPROVED` in the workflow (`RepairOrderNotApprovedForPaymentException` → 422 — you don't charge before the budget is approved). Fields: `amount` (`BigDecimal`/`NUMERIC(12,2)`), `method` (`entity/enums/PaymentMethod`: `CASH`, `CARD`, `PIX`, `BOLETO`, `MERCADO_PAGO_CHECKOUT`), `status` (`entity/enums/PaymentStatus`: `PENDING`, `APPROVED`, `REJECTED`, `REFUNDED`, `CANCELLED` — **not** a strict workflow), `installments` (>1 only for `CARD`; the service normalizes it to 1 otherwise), `payerName`/`payerDocument` (for the receipt), `externalReference` (`payment-<id>-<uuid>`, generated + persisted on the first checkout link — must be globally unique so a stale MP payment cannot false-approve it on sync), `gatewayPaymentId` (the real MP payment id, set on sync), `paidAt`. Dedicated `PATCH /payment/{id}/status` endpoint; setting status to `APPROVED` stamps `paidAt` (the repair order's own status is left untouched). `GET /payment/{id}/receipt` returns a non-fiscal PDF receipt (`service/PaymentReceiptService`, OpenPDF) with store data from `config/ShopProperties` (`shop.*`).
- **Payment ↔ Mercado Pago (Checkout Pro)** — `POST /payment/{id}/checkout` creates a Checkout Pro *preference* (`client/MercadoPagoClient.createCheckoutPreference`) for a **pending** `MERCADO_PAGO_CHECKOUT` payment and returns the `init_point` link; it also stamps `externalReference`. The preference includes a `payer` block (name, customer email, CPF/CNPJ when `payerDocument` is 11/14 digits) to help MP surface Pix and match the payment on sync — but Pix visibility ultimately depends on the seller's MP account having a Pix key. `POST /payment/{id}/sync` polls MP (`findPaymentByExternalReference`) and reapplies the mapped status (`InvalidPaymentCheckoutException` → 422 if no link yet; `PaymentGatewayException` → 502 if MP fails/unconfigured). No webhook (app not hosted); reconciliation is manual polling. The **physical card machine** (Mercado Pago **Point** API) is still a TODO in `MercadoPagoClient` — needs production credentials + a Point Smart device.
- **User** — `entity/User implements UserDetails`; `name`, unique `email`, `password` (hashed), `role` (`entity/enums/Role`: `ADMIN`, `GERENTE`, `ATENDENTE`, `TECNICO`, `PENDENTE`; defaults to `PENDENTE`) mapped to a single `ROLE_<name>` `GrantedAuthority`, `enabled` (defaults `true` at the DB/entity level, but `UserService.save` forces it to `false` for every self-registration — new accounts start locked out until an ADMIN activates them). An ADMIN manages users via `GET /user` (paginated + filterable by `name`/`email`/`role`/`enabled`), `PATCH /user/{id}/role` (role-only), and `PATCH /user/{id}/status` (enabled-only — used both to activate a new account and to suspend/reactivate an existing one). User management (`GET /user`, `PATCH /user/{id}/role`, `PATCH /user/{id}/status`) is **ADMIN-only** — not even GERENTE. See the Auth section for the full role→endpoint matrix; all gating is URL-based in `SecurityConfig`, not `@PreAuthorize` (this codebase has no method security).
- **Soft delete** — `DELETE` on `Brand`, `Accessory`, `Customer`, `Device` and `RepairOrder` is a soft delete: Hibernate `@SoftDelete(strategy = TIMESTAMP, columnName = "deleted_at")` on those 5 entities stamps `deleted_at` instead of removing the row and appends `deleted_at IS NULL` to every query (derived, `Specification`, `@EntityGraph`, and the `devices_accessories` join table). There is **no API path for permanent removal** — that's a manual DB operation. Services block soft-deleting a record that still has active children (`EntityInUseException` → 409): `Brand`←`Device`, `Accessory`←`Device`, `Customer`/`Device`←`RepairOrder`, `RepairOrder`←`Payment`.
- **`Payment` is the exception — physical delete.** It is the inverse side of `RepairOrder`'s eager `@OneToOne`; with `@SoftDelete` the join predicate makes Hibernate raise `FetchNotFoundException` for *every* repair order that has no payment, and its `UNIQUE repair_order_id` would let a logically-deleted row block a new payment. `PaymentService.deleteById` nulls `RepairOrder.payment` then hard-deletes.
- Migrations are append-only under `src/main/resources/db/migration` (currently V1–V20) — never edit an existing `V*.sql`, always add the next `V{n}`. Confirm the actual current max version with `ls` before citing a number; don't trust stale docs or memory.
...
- Authorization beyond "is authenticated" is role-gated in `SecurityConfig` via `requestMatchers(HttpMethod, path).hasAnyRole(...)` — **no `RoleHierarchy`**, each rule lists every allowed role explicitly, and the chain ends with `.anyRequest().denyAll()` so a role with no matching rule (notably `PENDENTE`) gets nothing. Roles: `ADMIN` (everything), `GERENTE` (like ADMIN minus user management; its deletes are soft like everyone's), `ATENDENTE` (recepção), `TECNICO` (chão de oficina), `PENDENTE` (no access, awaiting classification). Matrix (`A`=ADMIN `G`=GERENTE `T`=ATENDENTE `C`=TECNICO):
  - `Brand`/`Accessory`: `GET` → A G T; `POST` + `DELETE` → A G.
  - `Customer`/`Device`: `GET`/`POST`/`PUT` → A G T; `DELETE` → A G. Exception: `GET /device/{id}` (only the by-id detail — **not** the list nor `/device/serial-number`) → A G T C, so a TÉCNICO can open a device's details from a repair order.
  - `RepairOrder`: `GET` → A G T C; `POST`/`PUT` → A G T; `PATCH /{id}/status` → **the only lever for the status workflow (`PUT` ignores the body's `status`)** — URL-level it's A G T C (note ATENDENTE is on the URL matcher too), but that's a superset: `SecurityConfig` can't see the request body, so the per-target-status rule lives in `RepairOrderService.updateStatus` instead — TÉCNICO drives every transition except setting `DEVICE_COLLECTED` (aparelho retirado), and ATENDENTE is the mirror image, allowed only to set `DEVICE_COLLECTED` and blocked from every other transition (`RepairOrderStatusActorNotAllowedException` → 403). GERENTE/ADMIN are unrestricted. `DELETE` → A G.
  - `Payment` (incl. `checkout`/`sync`/`receipt`/`PATCH /{id}/status`): all → A G T; `DELETE` → A G (physical delete, not soft — see Domain model).
  - `GET /user`, `PATCH /user/{id}/role`, `PATCH /user/{id}/status` → A only.
- A newly self-registered user (`POST /auth/register`) starts `enabled=false` **and `role=PENDENTE`**, and cannot log in until an ADMIN activates the account (`PATCH /user/{id}/status`, `enabled:true`) and sets its role (`PATCH /user/{id}/role`). Login for a disabled account throws Spring Security's `DisabledException`, caught in `AuthController.login` and translated to `exception/AccountNotActivatedException` → 403. Suspending an already-active user is the same `PATCH /user/{id}/status` endpoint with `enabled:false`; the effect is not instant — `SecurityFilter` keeps trusting JWT claims only (no per-request DB lookup), so an already-issued access token stays valid until it expires naturally, but `RefreshTokenService.findValidToken` checks `user.isEnabled()` so the refresh token stops minting new access tokens right away, and a fresh login attempt is rejected immediately.
