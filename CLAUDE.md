...
- Migrations are append-only under `src/main/resources/db/migration` (currently V1–V17) — never edit an existing `V*.sql`, always add the next `V{n}`. Confirm the actual current max version with `ls` before citing a number; don't trust stale docs or memory.
