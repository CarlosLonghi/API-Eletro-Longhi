...
- Authorization beyond "is authenticated" is role-gated in `SecurityConfig` via `requestMatchers(HttpMethod, path).hasRole("ADMIN")` — ADMIN-only: `POST`/`DELETE /brand`, `POST`/`DELETE /accessory`, `DELETE /customer/{id}`, `DELETE /device/{id}`, `DELETE /repair-order/{id}`, `GET /user`, `PATCH /user/{id}/role`, `PATCH /user/{id}/status`. Everything else (GET/PUT/PATCH, plus `POST /customer`, `POST /device`, `POST /repair-order`) stays open to any authenticated USER or ADMIN.
